...
public class BeatReactor implements Closeable {
    ...
    public void addBeatInfo(String serviceName, BeatInfo beatInfo) {
        ...
        //fix #1733 again
        // 如果实例已经在map(dom2Beat)中已经存在会返回原值，如果不存在，即新实例会插入map，返回null
        if ((existBeat = dom2Beat.put(key, beatInfo)) != null) {
            existBeat.setStopped(true);
        }
        // 定期发送心跳包以维持服务实例的“活跃”状态。
        // 通过 ScheduledExecutorService 来调度 BeatTask，确保每隔一段时间（由 beatInfo.getPeriod() 决定）就执行一次心跳检查或更新操作。
    ...
    class BeatTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // 不论成功与否，在 finally 块中总是会安排一个新的 BeatTask 来确保心跳机制持续运行
